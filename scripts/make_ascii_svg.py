...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "rodri-ascii.svg")
...
CONTRAST = 1.12
BRIGHTNESS = 1.0
GAMMA = 1.02          # >1 brightens mids -> face lands in sparser chars
SHARPEN = True
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">rodri@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'rodri@github:~$ whoami <tspan fill="{INK}">Rodri Gonzalez</tspan></text>')
